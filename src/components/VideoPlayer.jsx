...
export default function VideoPlayer({ video, onClose }) {
  ...
  const MetaRow = ({ label, value, multiline }) => {
    ...
    return (
      <div className={`flex ${multiline ? 'items-start' : 'items-center'} py-1`}>
        <span className="text-white font-semibold w-28 md:w-36 shrink-0">{label}</span>
        <span className="text-white font-semibold w-4 shrink-0">:</span>
        <span className={`flex-1 min-w-0 ${multiline ? 'leading-relaxed' : 'truncate'} text-neutral-300`}>
          {finalValue}
        </span>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/95 overflow-y-auto custom-scrollbar">
      ...
      <div className="min-h-full flex flex-col items-center px-4 pt-20 pb-12">
        ...
        >
          <div className={`absolute -top-14 left-1/2 -translate-x-1/2 z-[60] flex justify-center transition-opacity duration-300 ${showCloseButton ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button 
              onClick={onClose} 
              className="
                bg-neutral-800/80 
            ...
            <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4 flex flex-col gap-2 transition-opacity duration-300 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <div className="flex items-center justify-between text-white" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-4">
                  <button onClick={togglePlay} className="text-white hover:text-[#f47521] transition-colors focus:outline-none p-1 cursor-pointer">
                    {isPlaying ? (
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <button onClick={toggleMute} className="text-white hover:text-[#f47521] transition-colors p-1 cursor-pointer">
                      {isMuted || volume == 0 ? (
                  ...
                  <button onClick={toggleFullscreen} className="text-white hover:text-[#f47521] transition-colors p-1 cursor-pointer">
                ...
                {video.performers.map((perf, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center bg-[#141519] rounded-lg overflow-hidden border border-neutral-800 hover:border-[#f47521] transition group p-2.5 w-full shadow-md">
                      {perfImage ? (
                        <img src={perfImage} alt={perf.name} className="w-full h-36 rounded object-cover object-top shrink-0 mb-2" />
                      ) : (
          ...
          <div className="flex-1 min-w-0 flex flex-col pt-1">
            <div className="mt-8">
              ...
              <div className="mt-4 pt-4 flex flex-col">
                <MetaRow label="Date" value={video.date} />
                <MetaRow label="Studio Code" value={video.code || video.fileInfo?.code} />
                <MetaRow label="Studio" value={video.studio} />
                <MetaRow label="Director" value={video.director} />
          ...
          return (
            <div onClick={() => setPreviewIndex(null)} className="fixed inset-0 bg-black/95 z-[99] flex items-center justify-center p-4 cursor-pointer select-none">
              ...
              <button 
                ...
                className="absolute left-6 text-white bg-neutral-800/80 hover:bg-[#f47521] hover:text-black w-12 h-12 rounded-full flex items-center justify-center transition-colors shadow-lg z-[100] cursor-pointer"
                title="Previous"
              >
                ❮
              </button>
              ...
              <div className="relative max-w-5xl max-h-screen flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={() => setPreviewIndex(null)} className="absolute -top-12 right-0 text-white bg-neutral-800 hover:bg-[#f47521] hover:text-black font-bold px-4 py-2 rounded-full transition-colors shadow-lg cursor-pointer">✕</button>
              </div>

              <button 
                onClick={handleNextImage} 
                className="absolute right-6 text-white bg-neutral-800/80 hover:bg-[#f47521] hover:text-black w-12 h-12 rounded-full flex items-center justify-center transition-colors shadow-lg z-[100] cursor-pointer"
                title="Next"
              >
                ❯
              </button>
